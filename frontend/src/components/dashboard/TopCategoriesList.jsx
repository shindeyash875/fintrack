...
const RANK_BADGE_CLASSES = [
  ...
  'bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-200 border-slate-300 dark:border-slate-600', // #2 Silver
  'bg-orange-100 dark:bg-orange-950/60 text-orange-800 dark:text-orange-300 border-orange-200 dark:border-orange-800', // #3 Bronze
...
export const TopCategoriesList = ({ categories = [], isLoading = false }) => {
  ...
  if (isLoading) {
    return (
      <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 sm:p-6 border border-slate-200/80 dark:border-slate-800 shadow-sm animate-pulse space-y-4">
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            ...
            className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 hover:text-emerald-700 dark:hover:text-emerald-300 inline-flex items-center gap-1 transition-colors min-h-[36px]"
          >
            Manage
        ...
        {!categories || categories.length === 0 ? (
          ...
          <div className="space-y-3.5">
            {categories.map((cat, idx) => {
              const rankClass =
                RANK_BADGE_CLASSES[idx] || 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-700';
              const barClass =
