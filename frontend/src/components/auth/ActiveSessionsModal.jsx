      ...
      <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-100 dark:border-slate-800 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-850/50">
          <div className="flex items-center gap-2.5">
            ...
            <h3 className="text-base font-bold text-slate-800 dark:text-white">Active Sessions</h3>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-4">
          ...
          {isLoading ? (
            ...
            <div className="space-y-2.5 max-h-60 overflow-y-auto">
              {sessions.length === 0 ? (
                <p className="text-xs text-slate-500 dark:text-slate-400 text-center py-4">No other active sessions detected.</p>
              ) : (
                sessions.map((sess) => (
                  <div
                    key={sess.id}
                  ...
                  >
                    ...
                    <span className="text-[10px] font-medium text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/50 px-2 py-0.5 rounded-full border border-emerald-100 dark:border-emerald-800">
                      Active
                    </span>
                  </div>
                ))
